...
from common.exceptions                      import NiceAPIException, DataEncryptoException


class NiceAPI:
    NICE_API_HOST               = settings.NICE_API_HOST
    NICE_URI_ISSUE_TOKEN        = settings.NICE_URI_ISSUE_TOKEN
    NICE_URI_ISSUE_CRYPTO_TOKEN = settings.NICE_URI_ISSUE_CRYPTO_TOKEN
    NICE_URI_REVOKE_TOKEN       = settings.NICE_URI_REVOKE_TOKEN
    NICE_CLIENT_ID              = settings.NICE_CLIENT_ID
    NICE_CLIENT_SECRET          = settings.NICE_CLIENT_SECRET
    NICE_PRODUCT_ID             = settings.NICE_PRODUCT_ID
    CONTENT_TYPE_X_WWW          = 'application/x-www-form-urlencoded;charset=utf-8'
    CONTENT_TYPE_AP_JSON        = 'application/json'
    ...
    def issue_institution_token(self) -> dict:
        ...
        """
        url = self.NICE_API_HOST + self.NICE_URI_ISSUE_TOKEN
        token_auth_key = base64.b64encode(f'{self.NICE_CLIENT_ID}:{self.NICE_CLIENT_SECRET}'.encode()).decode()
        ...
        if data_header['GW_RSLT_CD'] != '1200':
            raise NiceAPIException(data_header['GW_RSLT_MSG'])
    ...
    def discard_institution_token(self, access_token: str) -> None:
        """ NICE API 기관용 토큰 폐기
            ...
            return
            - tuple(bool, str) (요청 성공시 True, NiceAPI 요청 실패 시 False)
        """
        url = self.NICE_API_HOST + self.NICE_URI_REVOKE_TOKEN
        
        current_timestamp = str(int(get_time.time()))
        token_auth_key = base64.b64encode(f'{access_token}:{current_timestamp}:{self.NICE_CLIENT_ID}'.encode()).decode()
    ...
    def issue_crypto_token(self, access_token):
        """ NICE API 암호화 토큰 발행
            ...
            *return
            - crypto_token_data (NICE 표준창 서비스 요청에 필요한 data set)
        """
        url = self.NICE_API_HOST + self.NICE_URI_ISSUE_CRYPTO_TOKEN
        
        current_timestamp = str(int(get_time.time()))
        crypto_token_auth_key = base64.b64encode(f'{access_token}:{current_timestamp}:{self.NICE_CLIENT_ID}'.encode()).decode()
        
        req_dtim = datetime.today().strftime('%Y%m%d%H%M%S')
        req_no = f"{'Req'}{datetime.today().strftime('%Y%m%d%H%M%S%f')}"
        
        headers = {
            'Content-Type' : self.CONTENT_TYPE_AP_JSON,
            'Authorization': f'bearer {crypto_token_auth_key}',
            'productID'    : self.NICE_PRODUCT_ID,
        }
...
class AESCipherCBC:
    ...
    def decrypt_data(self, enc_data: str, key: bytes, iv: bytes):
        ...
        try:
            url_decoded_data: str = parse.unquote(enc_data)
